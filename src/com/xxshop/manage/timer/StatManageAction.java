 package com.xxshop.manage.timer;
...
import com.xxshop.foundation.domain.Group;
...
import com.xxshop.foundation.service.IActivityGoodsService;
import com.xxshop.foundation.service.IActivityService;
import com.xxshop.foundation.service.IDeliveryGoodsService;
...
import com.xxshop.foundation.service.IMobileVerifyCodeService;
import com.xxshop.foundation.service.IOrderFormLogService;
import com.xxshop.foundation.service.IOrderFormService;
import com.xxshop.foundation.service.IPaymentService;
import com.xxshop.foundation.service.IPredepositLogService;
import com.xxshop.foundation.service.IStoreClassService;
import com.xxshop.foundation.service.IStorePointService;
import com.xxshop.foundation.service.IStoreService;
import com.xxshop.foundation.service.IStoreStatService;
...
import com.xxshop.manage.admin.tools.StatTools;
     ...
     for (Store store : stores) {
       ...
       for (Evaluate eva : evas)
       {
         ...
         if ((CommUtil.null2Double(eva.getShip_evaluate()) < 0.0D) || 
           (CommUtil.null2Double(eva.getShip_evaluate()) >= 1.0D)) continue;
    ...
    // double service_evaluate;
     ...
     for (Group group : groups) {
       ...
       if (group.getEndTime().before(new Date())) {
         group.setStatus(-2);
         this.groupService.update(group);
//         for (service_evaluate = group.getGg_list().iterator(); service_evaluate.hasNext(); ) {
					for (Iterator ggg = group.getGg_list().iterator(); ggg
					.hasNext();) {
					gg = (GroupGoods)ggg.next();
           gg.setGg_status(-2);
     ...
     for (Activity act : acts) {
       ...
       for (ActivityGoods ac : act.getAgs()) {
         ...
         goods1.setGoods_current_price(goods.getStore_price());
...
//     PredepositLog log;
       ...
       if (ret) {
         ...
         if (of.getPayment().getMark().equals("balance")) {
           ...
           if (this.configService.getSysConfig().getBalance_fenrun() == 1)
           {
             ...
             buyer1.setFreezeBlance(BigDecimal.valueOf(
               CommUtil.subtract(buyer1.getFreezeBlance(), of
               .getTotalPrice())));
     ...
     stores = this.storeService
       ...
       params, -1, -1);
     for (Store store : stores) {
				for (Iterator iter_goods = store.getGoods_list().iterator(); iter_goods
				.hasNext();) {
					Goods goods4 = (Goods)iter_goods.next();
         if (goods4.getCombin_status() != 0) {
           goods4.setCombin_begin_time(null);
           goods4.setCombin_end_time(null);
           goods4.setCombin_price(null);
           goods4.setCombin_status(0);
           goods4.getCombin_goods().clear();
           this.goodsService.update(goods4);
         }
       }
     }
     ...
     List<Goods> goods_list = this.evaluateService.query_goods(
       ...
       -1, -1);
     double description_evaluate1;
     for (Goods goods3 : goods_list) {
       description_evaluate1 = 0.0D;
       params.clear();
       params.put("evaluate_goods_id", goods3.getId());
       ...
       for (Evaluate eva : eva_list) {
         description_evaluate1 = CommUtil.add(eva
           .getDescription_evaluate(), Double.valueOf(description_evaluate1));
       }
 
       description_evaluate1 = CommUtil.div(Double.valueOf(description_evaluate1), 
         Integer.valueOf(eva_list
         .size()));
       goods3.setDescription_evaluate(
         BigDecimal.valueOf(description_evaluate1));
       this.goodsService.update(goods3);
     }
   ...
   private boolean send_email(OrderForm order, String mark) throws Exception
   {
     ...
     if (template.isOpen()) {
       ...
       String path = System.getProperty("xxshop.root") + "vm" + 
         File.separator;
       ...
       p.setProperty("file.resource.loader.path", 
         System.getProperty("xxshop.root") + 
         "vm" + File.separator);
